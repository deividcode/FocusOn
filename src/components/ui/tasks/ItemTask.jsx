...
export const ItemTask = (props) => {
  const { id, name, description, amountPomodoros, status } = props.task;
  const { changeStatusTask, deleteTask } = useContext(TaskContext);
  ...
  return (
    <>
      <li className="grid gap-1 px-3 py-3 bg-electric-violet-200 dark:bg-[#0B1215] text-sm rounded-md">
        <div className="flex gap-8 justify-between items-start">
         <div className="w-full flex justify-between items-start">
          ...
          <div className="flex gap-1.5 items-center">
            {amountPomodoros}
            <svg
              className="w-5 fill-electric-violet-600"
              width="800px"
              height="auto"
              viewBox="0 0 24 24"
